...
# TODO Try out the example scripts in panda

# Test what causes the dashboard to throw up warning signs when running this script
#   Just connecting or can_recv
...
def receive():
  ...
  while(True):
    ...
    for rx_addr, rx_data_bytearray, rx_bus in messages:
      can_data.append(CanData(rx_addr, rx_data_bytearray, rx_bus))
      # print(f"{rx_addr=}")

    can_data = [int(time.monotonic() * 1e9), can_data]

    # can_packets: list[tuple[int, list[CanData]]]
    parser.update(can_data)
    # print(parser.vl["ESC_51"]["VL_Radgeschw"])
