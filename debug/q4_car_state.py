...
class CarState():
  ...
  def update(self, can_parsers: dict[StrEnum, CANParser]): # -> structs.CarState:
    ...
    self.vehicle_speed = pt_cp.vl["Kombi_01"]["KBI_angez_Geschw"] * CV.KPH_TO_MS
    ...
    self.steeringPressed  = abs(self.steeringTorque) > self.CCP.STEER_DRIVER_ALLOWANCE
    self.steeringSlightlyPressed = abs(self.steeringTorque) > self.CCP.STEER_DRIVER_SLIGHT_PRESS
    ...
    if self.CP.flags & VolkswagenFlags.ALT_GEAR:
      self.gearShifter = self.parse_gear_shifter(self.CCP.shifter_values.get(pt_cp.vl["Gateway_73"]["GE_Fahrstufe"], None)) # (candidate for all plattforms MEB and MQB evo)
    else:
      self.gearShifter = self.parse_gear_shifter(self.CCP.shifter_values.get(pt_cp.vl["Getriebe_11"]["GE_Fahrstufe"], None))
    self.drive_mode = self.gearShifter == GearShifter.drive
    ...
    self.parkingBrake = pt_cp.vl["ESC_50"]["EPB_Status"] in (1, 4) # EPB closing or closed (candidate for all plattforms)
    ...
    self.seatbeltUnlatched = pt_cp.vl["Airbag_02"]["AB_Gurtschloss_FA"] != 3
    ...
    self.cruiseState.available = pt_cp.vl["Motor_51"]["TSK_Status"] in (2, 3, 4, 5)
    self.cruiseState.enabled   = pt_cp.vl["Motor_51"]["TSK_Status"] in (3, 4, 5)
    ...
    if self.CP.flags & VolkswagenFlags.MEB:
      ...
      # EV battery details
      self.batteryDetails.charge = pt_cp.vl["Motor_16"]["MO_Energieinhalt_BMS"] # battery charge WattHours
      if self.CP.networkLocation == NetworkLocation.gateway:
        self.batteryDetails.heaterActive = bool(alt_cp.vl["MEB_HVEM_03"]["PTC_ON"]) # battery heater active
        self.batteryDetails.voltage      = alt_cp.vl["MEB_HVEM_01"]["Battery_Voltage"] # battery voltage
        self.batteryDetails.capacity     = alt_cp.vl["BMS_04"]["BMS_Kapazitaet_02"] * self.batteryDetails.voltage # EV battery capacity WattHours
        self.batteryDetails.soc          = self.batteryDetails.charge / self.batteryDetails.capacity * 100 if self.batteryDetails.capacity > 0 else 0 # battery SoC in percent
        self.batteryDetails.power        = alt_cp.vl["MEB_HVEM_01"]["Engine_Power"] # engine power output
        self.batteryDetails.temperature  = alt_cp.vl["DCDC_03"]["DC_Temperatur"] # dcdc converter temperature
